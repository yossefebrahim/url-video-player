...
class DiscoveryManagerMethodChannel : FlutterPlugin, MethodChannel.MethodCallHandler {
    ...
    fun selectRoute(id: String) {
        val routes = router?.routes ?: return
        // A device can be advertised by MORE THAN ONE route: the real Cast route
        // from CastMediaRouteProvider, and — on some devices (e.g. Samsung with
        // Smart View, or a lingering remote-display session) — a phantom
        // ROUTE_TYPE_USER route carrying the same CastDevice bundle. A plain
        // `find` returns whichever comes first, and if that's the user route,
        // `selectRoute` never starts a CAF session (onSessionStarted never
        // fires) so the connection just times out. Prefer a genuine Cast route:
        // one that supports the Cast live-audio/video control category.
        val matches = routes.filter {
            CastDevice.getFromBundle(it.extras)?.deviceId == id
        }
        // categoryForCast throws on a blank id, so coalesce null OR blank to the
        // default receiver id before building the category.
        val castAppId = CastContextMethodChannel.appId?.takeIf { it.isNotBlank() }
            ?: CastMediaControlIntent.DEFAULT_MEDIA_RECEIVER_APPLICATION_ID
        val castCategory = CastMediaControlIntent.categoryForCast(castAppId)
        val selectedRoute = matches.firstOrNull { it.supportsControlCategory(castCategory) }
            ?: matches.firstOrNull {
                it.supportsControlCategory(MediaControlIntent.CATEGORY_REMOTE_PLAYBACK)
            }
            ?: matches.firstOrNull()
        if (selectedRoute != null) {
            Log.d(TAG, "selectRoute($id): ${matches.size} match(es); chose ${selectedRoute.name} (${selectedRoute.supportedTrait()})")
            this.router?.selectRoute(selectedRoute)
        }
    }

    /** Small helper to describe a route's Cast-ness in logs. */
    private fun MediaRouter.RouteInfo.supportedTrait(): String = when {
        supportsControlCategory(MediaControlIntent.CATEGORY_REMOTE_PLAYBACK) -> "remote-playback"
        else -> "generic"
    }
